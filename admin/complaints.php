		...
		<div class="card">
			...
			<div class="card-body">
				<table class="table table-bordered table-hover" id="complaint-tbl">
					...
					<tbody>
						<?php
						$status = array("", "",  "Received", "Action Made");
						$qry = $conn->query("SELECT * FROM complaints where status != 1 order by unix_timestamp(date_created) desc ");
						while ($row = $qry->fetch_array()) :
						?>
							<tr class="<?php echo $row['status'] == 0 ? 'border-alert' : '' ?>">
			...
			<div class="card-body">
				<table class="table table-bordered table-hover" id="complaint-tbl">
					...
					<tbody>
						...
						$qry = $conn->query("CALL `getpending`(); ");
						while ($row = $qry->fetch_array()) :
						?>
